...
mod tests {
    ...
    fn test_build_list_fom_vec_1() {
        let nodes = vec![];
        let head: Option<Box<ListNode>> = ListNode::build_list_from_vec(nodes);
        assert_eq!(ListNode::build_vec_from_list(head), []);
    }
    ...
    fn test_build_list_fom_vec_2() {
        ...
        let head: Option<Box<ListNode>> = ListNode::build_list_from_vec(nodes);
        assert_eq!(ListNode::build_vec_from_list(head), [1, 2, 3, 4, 5]);
    }
}
